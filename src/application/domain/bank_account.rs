...
mod tests {
    ...
    fn it_should_create_an_account_wth_default_account_balance() {
        let account = BankAccount::new("__BANK_ACCOUNT_ID__".to_string(), FULLNAME.to_string(), 100);
        assert_eq!(account.account_balance, 100);
    }

    #[test]
    fn it_should_be_able_to_add_funds() {
        let mut account = BankAccount::new("__BANK_ACOUNT_ID__".to_string(), FULLNAME.to_string(), 100);
